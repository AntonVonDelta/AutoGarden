...
//https://github.com/toomasz/FixedString
...
public:
	...
	bool prepend(const char* str){
		...
		int last_index=length()+len-1;
		for(int i =0; i < length(); i++)
		{
			_str()[last_index-i] = _str()[length()-1-i];
		}
	...
	void trimEnd(char c)
	{
		...
		while(charactersToTrim < length() && c_str()[max_index-charactersToTrim] == c)
		{
			charactersToTrim++;
		}
		_length -= charactersToTrim;
...
public:
	...
	FixedString(String str) :FixedStringBase(N)
	{
